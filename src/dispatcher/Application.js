...
export class Application {
	constructor() {
		...
		Object.keys(LogLevel).forEach((level) => {
			...
			this.$Logger[handler] = (...args) => {
				ref.getAllWindow().forEach((win) => win.webContents.$Scope.$Logger[handler](args));
			};
		});
		Object.freeze(this);
	}
	createWindow(name, config) {
		...
		let scope = win.webContents.$Scope = {
			...
			send:(handler,...args) =>{ win.webContents.send(handler,args); },
			$Logger:{}
		}
	...
	closeAll() {
		Object.keys(this.$Windows).forEach((x) => {
			let targ = this.$Windows[x];
			if (
				targ instanceof BrowserWindow &&
				targ.isDestoryed &&
				!targ.isDestoryed()
			) {
				targ.close();
